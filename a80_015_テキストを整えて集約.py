...
message_human = '''
# 命令文  : 本文1～本文5を参照し、ヘッダー行およびフッター行をみつけてください。つまり、最初数行または最後数行に繰り返し出てくる同じような行のことです。
           それらヘッダー行・フッター行の正規表現を作成してください。
# 出力    : ヘッダー行・フッター行の正規表現のみを出力してください。 
# 補足    : 先頭行や最終行で、数字と記号のみの行もヘッダー行・フッター行として扱ってください。 
...
class HeaderFooter(BaseModel):
    ...
    hdft_sample: str = Field(..., description="ヘッダー行・フッター行の代表サンプル")
...
class HeaderFootersGenerator:
    ...
    def read_random_wt_file(self, folder_path):
        ...
        if not wt_files:
            return None, "No *_WT.txt files found in the specified folder."
        
        # ランダムにファイルを選択
        random_file = random.choice(wt_files)
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                content = file.read()
            return random_file, content
        except Exception as e:
            return None, f"Error reading file {random_file}: {str(e)}"
    ...
    def run(self, folder_temp , state_header_footer) -> HeaderFooters:
        prompt = ChatPromptTemplate.from_messages([
            ("system", "あなたはテキストを分析する優秀なアシスタントです。"),
            ("human" , message_human),
            ])

        if state_header_footer == ['', '', ''] :  # ヘッダー、フッターの正規表現が未指定の場合

            # ランダムに５つのファイルを選択して読み込む
            sample_txt =[]
            for i in range(5):
                file_name, file_content = self.read_random_wt_file(folder_temp)
                sample_txt.append(file_content)
            ...
            HeaderFooter_sample = [hdft.hdft_sample for hdft in hdfts.hdfts] 
            print(HeaderFooter) 
            print(HeaderFooter_sample)

        else :  # ヘッダー、フッターの正規表現が指定されている場合
            HeaderFooter = state_header_footer

        # folder_tempリストの内容をファイルに出力する
        with open(folder_temp + r'/header_footer.txt', 'w' , encoding='utf-8') as f:
        ...
        return HeaderFooter, combined_text
...
if __name__ == "__main__":
    ...
    folder_temp = r'./kk_kai_20250104_162322/'   # フォルダ内の全てのWT.txtからヘッダー、フッター、文頭の空行、文末の空行を削除し、同フォルダに WT_cleaned.txtを作成する 
    #folder_temp = r'./doc_raw/kana_0102/'   # フォルダ内の全てのWT.txtからヘッダー、フッター、文頭の空行、文末の空行を削除し、同フォルダに WT_cleaned.txtを作成する 
    ...
    hdfts: HeaderFooters = hdft_generator.run(folder_temp =folder_temp ,state_header_footer = ['^\s*公共建築改修工事標準仕様書（電気設備工事編）令和４年版\s*$', '^\s*\d+\s*国土交通省大臣官房官庁営繕部\s*$', '^\s*$'])
