...
#model = torch.hub.load("./",'yolov5s'  , 'custom' , path='best11.pt' ,source='local' )#--- localのyolov5sを使用
#model = torch.hub.load( "./", 'custom', path='best_goochokipar_300epoc.pt', source='local')
model = torch.hub.load( "./", 'custom', path='01neji_best.pt', source='local')


#--- 検出の設定 ---
model.conf = 0.3 #--- 検出の下限値（<1）。設定しなければすべて検出
model.classes = [3] #--- 0:person クラスだけ検出する。設定しなければすべて検出
#print(model.names) #--- （参考）クラスの一覧をコンソールに表示

#model.weight = 'best_goochokipar_300epoc.pt'

#model = torch.hub.load('ultralytics/yolov5', 'yolov5s', classes=10)  # create model
#ckpt = torch.load('best_goochokipar_300epoc.pt')['model']  # load checkpoint
#model.load_state_dict( torch.load('best_goochokipar_300epoc.pt')['model'].state_dict()  )
